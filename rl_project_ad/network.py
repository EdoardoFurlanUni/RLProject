...
class Network(nn.Module):
    """
        Take a state, pass it through a one-hidden layer network and output the action values.
    """
...
class DQNAgent:
    """
    Implementation for Vanilla DQN Agent
    """
    ...
    def __init__(
        ...
        self.network = Network(state_dim, action_dim).to(self.device) #network to train

        self.target_network = Network(state_dim, action_dim).to(self.device) #target network
    ...
    def epsilon(self):
        """Current epsilon (linear decay).""" #questo va guardato meglio
    ...
    def train_step(self):
        ...
        #    q_network(states) → shape (batch, 5)
        #    .gather(1, actions.unsqueeze(1)) → picks the Q-value at index 'action'
        #    .squeeze(1) → back to shape (batch,)
        q_values = self.network(states).gather(1, actions.unsqueeze(1)).squeeze(1)
        # 3. Target: r + gamma * max_a' Q_target(s', a') * (1 - done)
        with torch.no_grad():
