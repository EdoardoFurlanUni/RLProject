...
\begin{abstract}
This report investigates the application of deep reinforcement learning (RL) algorithms for autonomous vehicle control in a dense highway driving scenario using the open-source \texttt{HighwayEnv} simulator. In this first part, we define the kinematic decision-making problem and compare the performance of a standard Deep Q-Network (DQN) agent against manual human control and rule-based physical baselines. The initial results show that while the vanilla DQN agent successfully learns a collision-free policy (yielding a 0\% crash rate), it results in an overly cautious driving style characterized by minimal speeds and high lateral instability. This preliminary analysis highlights the necessity of reward shaping techniques and neural architecture enhancements (such as Double DQN) which will be explored in the bonus section.
\end{abstract}

\section{Introduction to the Problem}

Highway autonomous driving is an overall very complex task for the agent. Unlike other scenarios, here the environment poses a great challenge due to the many kind of unknown variables from the surrounding: the goal is then to quickly adapt to different conditions while maintaining the driving objectives like speed and desired lane. To simulate the environment, in this work I employ the \texttt{highway-v0} environment from the \texttt{HighwayEnv} simulator \cite{highway-env}.
...
\begin{itemize}
    \item \textbf{Base DQN} achieves exceptional safety with a $0\%$ crash rate and survival for the full 40 steps. However, this safety is the result of an overly cautious driving style: the agent never accelerates, maintaining an average speed of $20.03$ m/s which is indeed very close to the minimum allowed speed. Additionally, the policy is laterally unstable, resulting in $22.1$ lane changes per episode (zig-zagging constantly). The results indicate that the network is able to avoid collisions but at the expense of an extremely safe driving style.
...
\begin{itemize}
    ...
    \item \textbf{State representation limits}: The standard flattened $5 \times 5$ kinematics grid lacks direct spatial and safety context, forcing the network to guess relative lane layouts and safety margins. This suggests that an explicit enrichment of the state representation is needed to give the network more context and improve training efficiency.
...
To align the agent's objective with safety and lateral comfort, I designed a custom \texttt{ShapedRewardWrapper} that modifies the reward function used internally by the agent during training. This is done by adding penalty terms to the baseline feedback to discourage frequent lane changes and dangerous tailgating. Crucially, this shaped reward is only used internally during the training phase to guide policy learning; during testing and evaluation, the agent is evaluated using the completely standard, unmodified environment reward function ($R_{env}$) to guarantee a fair comparison. I define the shaped reward function $R_{\text{shaped}}$ as:
\begin{equation}
\begin{split}
...
Additionally, I investigated the effect of a slower exploration decay ($\epsilon$-decay over 10,000 steps instead of 5,000) under the strict reward shaping configuration. The results showed that slowing down the exploration decay did not yield significant improvements in this phase: the agent's speed remained close to the minimum limit while still registering minor collision risks, indicating that reward shaping alone is insufficient to learn a high-speed, collision-free policy.
...
The absolute lane index allows the network to map lane coordinates. The Time-to-Collision is calculated as:
\begin{equation}
...
\begin{itemize}
    \item \textbf{P4.1: DDQN (Mis)}: The initial Double DQN model trained on the default 4-lane layout but evaluated on the denser 3-lane scenario. This configuration demonstrates that the Double DQN network allows the agent to achieve a significantly higher velocity ($23.34$ m/s) compared to standard DQN; however, it remains highly accident-prone, suffering a $40.0\%$ crash rate due to the spatial mismatch between training and testing road geometries.
    \item \textbf{P4.2: DDQN (3L)}: Resolving the layout mismatch by training directly on a 3-lane setup. This configuration achieves absolute safety ($0.0\%$ crash rate) and high return ($29.13$), but remains somewhat conservative with an average speed of $20.90$ m/s and very few lane changes ($2.50$).
    \item \textbf{P4.3: DDQN (Bal)}: Trained with tuned trade-off parameters (safety distance $0.28$, proximity penalty $0.35$, speed reward $0.45$) to encourage higher velocity. The agent successfully drives at $22.25$ m/s with $0.0\%$ crashes. However, it displays a "lane-stickiness" behavior: when blocked by a slow vehicle while driving in the rightmost lane, it stays tailgating rather than performing a clean overtake because the lane change penalty ($-0.20$ total) is too high compared to the proximity penalty.
    \item \textbf{P4.4: DDQN (Tuned)}: A refined configuration to promote active overtaking and prompt return to the right-most lane. To achieve this, the right lane reward is increased to $0.20$ to incentivize returning to the right lane, while the total lane change penalty is kept moderate at $-0.12$ ($-0.06$ environment + $-0.06$ wrapper) to prevent aimless zig-zagging. Proximity parameters are kept at a safety distance of $0.25$ and penalty of $0.25$ to maintain a brave, close-driving style. This configuration achieves an average speed of $21.65$ m/s, $0.0\%$ crash rate, and very stable lane selection (averaging only $2.30$ lane changes per episode).
\end{itemize}
...
\begin{table}[ht]
\caption{Comparison of average performance for all configurations over 10 evaluation episodes.}
\label{tab:bonus-results}
\vskip 0.15in
\begin{center}
\begin{small}
\renewcommand{\arraystretch}{1.3}
...
\midrule
P3.1: Short State & 29.43 & 40.00 & 0.0\% & 13.30 & 20.38 \\
P3.2: State (Lg) & 26.98 & 37.20 & 10.0\% & 1.20 & 20.69 \\
\midrule
P4.1: DDQN (Mis) & 26.17 & 32.80 & 40.0\% & 4.80 & 23.34 \\
P4.2: DDQN (3L) & 29.13 & 40.00 & 0.0\% & 2.50 & 20.90 \\
P4.3: DDQN (Bal) & 31.38 & 40.00 & 0.0\% & 9.70 & 22.25 \\
...
\section{Conclusions}
In this project, we investigated the design of deep reinforcement learning policies for autonomous highway driving. Starting with a basic DQN agent, we addressed its safety and velocity limitations through a structured progression of reward shaping, environmental geometry matching, state representation enrichment, and algorithmic upgrades using Double DQN. 

Our findings indicate that vanilla DQN suffers from maximization bias and safety generalization gaps when transferred between different lane geometries. While custom reward shaping and state representation modifications successfully mitigated collision risks, they often resulted in overly conservative behaviors (lane stickiness and low velocities). Upgrading to Double DQN successfully decoupled action selection from evaluation, stabilizing training and allowing the network to learn a much higher-speed policy. By carefully balancing the trade-offs between speed, right-lane preference, and lane change penalties in our final courageous configuration, we obtained a policy that actively and safely overtakes slower vehicles while promptly returning to the right-most lane, achieving high returns and a $0\%$ crash rate.
